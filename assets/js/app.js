(function () {
  ...
  const state = {
    ...
    playbackTimes: JSON.parse(localStorage.getItem('playbackTimes') || '{}'),
  ...
  const els = {
    ...
    technologyPage: document.getElementById('technologyPage'),
    sciencePage: document.getElementById('sciencePage'),
    geographyPage: document.getElementById('geographyPage'),
    ...
    continueListeningContent: document.getElementById('continueListeningContent'),
    ...
    recentlyPlayedContent: document.getElementById('recentlyPlayedContent'),
    ...
    technologyBackBtn: document.getElementById('technologyBackBtn'),
    scienceBackBtn: document.getElementById('scienceBackBtn'),
    geographyBackBtn: document.getElementById('geographyBackBtn'),
    historyBackBtn: document.getElementById('historyBackBtn'),
    technologyContent: document.getElementById('technologyContent'),
    scienceContent: document.getElementById('scienceContent'),
    geographyContent: document.getElementById('geographyContent'),
    ...
    miniSpeedBtn: document.getElementById('miniSpeedBtn'),
    ...
    miniSkipBackBtn: document.getElementById('miniSkipBackBtn'),
    miniSkipForwardBtn: document.getElementById('miniSkipForwardBtn'),
    miniSpeedBtn: document.getElementById('miniSpeedBtn')
  };

  document.body.appendChild(els.detailModal);
  document.body.appendChild(els.addModal);
  ...
  function openPage(page) {
    ...
    const isCategory = ['technology', 'science', 'geography', 'history'].includes(page);

    document.body.classList.toggle('ai-mode', isAI);
    els.homePage.classList.toggle('active', !isAI && !isAbout && !isCategory);
    els.aiPage.classList.toggle('active', isAI);
    els.aboutPage.classList.toggle('active', isAbout);
    els.technologyPage.classList.toggle('active', page === 'technology');
    els.sciencePage.classList.toggle('active', page === 'science');
    els.geographyPage.classList.toggle('active', page === 'geography');
    els.historyPage.classList.toggle('active', page === 'history');
  ...
  function renderHeroCards() {
    els.heroRow.innerHTML = categories.map((category) => `
      <article
        ...
        role="button"
        tabindex="0"
        aria-label="Open ${escapeHtml(category.id)} category"
    ...
    els.heroRow.querySelectorAll('[data-category]').forEach((card) => {
      const openCategory = () => {
  ...
  function getSearchSuggestions(query) {
    ...
    state.podcasts.forEach(podcast => {
      if (podcastMatchesQuery(podcast, query)) {
        const title = normalize(podcast.title);
        if (title.includes(cleanQuery) && !seen.has(podcast.id)) {
          suggestions.push({
            type: 'episode',
            podcast,
            text: podcast.title,
            meta: `${podcast.category} · ${podcast.duration}`
          });
          seen.add(podcast.id);
        }
      }
    ...
    categories.forEach(category => {
      const catName = normalize(category.id);
      if (catName.includes(cleanQuery) && !seen.has(category.id)) {
        suggestions.push({
          type: 'category',
          category,
          text: category.id,
  ...
  function renderSearchSuggestions(suggestions) {
    ...
    els.searchSuggestions.innerHTML = suggestions.map(suggestion => `
      <div class="search-suggestion-item" data-type="${suggestion.type}" data-id="${suggestion.type === 'episode' ? suggestion.podcast.id : suggestion.category.id}">
        <div class="search-suggestion-title">${highlightText(suggestion.text, els.searchInput.value)}</div>
        <div class="search-suggestion-meta">${escapeHtml(suggestion.meta)}</div>
      </div>
    `).join('');
  ...
  function renderPodcastCard(podcast) {
    ...
    const isFavorite = state.favorites.includes(podcast.id);
    return `
      <article
        class="card-surface"
      ...
      >
        <div class="cover" style="background:${getCoverGradient(podcast)};">
          ...
          <button class="favorite-btn ${isFavorite ? 'active' : ''}" data-favorite="${escapeHtml(podcast.id)}" aria-label="${isFavorite ? 'Remove from' : 'Add to'} favorites">
            ${isFavorite ? '⭐' : '☆'}
          </button>
        </div>
        <div class="card-body">
          <div class="title-row">
            <h3 class="episode-title">${highlightText(podcast.title, query)}</h3>
            <div class="duration">${escapeHtml(podcast.duration)}</div>
          </div>
          <div class="meta">${highlightText(podcast.category, query)} · ${highlightText(podcast.description, query)}</div>
          <div class="chip-wrap">
            ${shortTags(podcast.tags.map(tag => highlightText(tag, query)))}
          </div>
        </div>
      </article>
    `;
  ...
  function bindLibraryCards() {
    const containers = [
      els.librarySections,
      els.continueListeningContent,
      els.recentlyPlayedContent,
      els.favoritesContent,
      els.technologyContent,
      els.scienceContent,
      els.geographyContent,
      els.historyContent
    ];
    ...
    document.querySelectorAll('[data-favorite]').forEach((btn) => {
      btn.addEventListener('click', (event) => {
        ...
        const podcastId = btn.getAttribute('data-favorite');
  ...
  function renderLibrary() {
    const query = els.searchInput.value;
    const filteredPodcasts = state.podcasts.filter((podcast) => podcastMatchesQuery(podcast, query));

    if (!filteredPodcasts.length) {
      els.librarySections.innerHTML = `
        <div class="library-empty">
          <h3>No podcasts found</h3>
          <p>Try searching for a different title, category, or tag.</p>
        </div>
      `;
      return;
    }
    ...
    filteredPodcasts.forEach((podcast) => {
      const categoryKey = categories.some((cat) => normalize(cat.id) === normalize(podcast.category))
        ? categories.find((cat) => normalize(cat.id) === normalize(podcast.category)).id
        : podcast.category;
      if (!groupedByCategory[categoryKey]) {
        groupedByCategory[categoryKey] = [];
      }
      groupedByCategory[categoryKey].push(podcast);
    });

    els.librarySections.innerHTML = Object.entries(groupedByCategory)
        ...
        return `
          <section class="library-category">
            <div class="section-header">
              <div>
                <h2 class="section-title">${escapeHtml(categoryId)}</h2>
                <p class="section-hint">${escapeHtml(category?.desc || 'Latest episodes')}</p>
              </div>
            </div>
  ...
  function renderCategoryPage(categoryId) {
    const content = els[`${categoryId}Content`];
    if (!content) return;

    const categoryPodcasts = state.podcasts.filter((podcast) => normalize(podcast.category) === normalize(categoryId));

    if (!categoryPodcasts.length) {
      content.innerHTML = `
        <div class="library-empty">
          <h3>No episodes yet</h3>
          <p>There are no podcasts in this category right now.</p>
        </div>
      `;
      return;
    }
    ...
    content.innerHTML = `
      <div class="library-grid">
        ${categoryPodcasts.map(renderPodcastCard).join('')}
      </div>
    `;
  ...
  function savePlaybackTime(podcastId, time) {
    state.playbackTimes[podcastId] = time;
    localStorage.setItem('playbackTimes', JSON.stringify(state.playbackTimes));
  }

  function getPlaybackTime(podcastId) {
    return state.playbackTimes[podcastId] || 0;
  }

  function addToRecentlyPlayed(podcast) {
    if (!podcast || !podcast.id) return;

    state.recentlyPlayed = [podcast.id, ...state.recentlyPlayed.filter((id) => id !== podcast.id)]
      .slice(0, 10);
  ...
  function renderContinueListening() {
    const continueItems = Object.entries(state.playbackTimes)
      .filter(([id, time]) => time > 30) // Only show if more than 30 seconds played
      .sort(([, a], [, b]) => b - a) // Sort by most recent
      .slice(0, 3) // Show up to 3
      .map(([id]) => state.podcasts.find(p => p.id === id))
      .filter(Boolean);

    if (!continueItems.length) {
      els.continueListeningSection.style.display = 'none';
      return;
    }

    els.continueListeningContent.innerHTML = continueItems.map(podcast => {
      const time = getPlaybackTime(podcast.id);
      const progress = (time / durationToSeconds(podcast.duration)) * 100;
      return `
        <article
          class="card-surface continue-card"
          data-open="${escapeHtml(podcast.id)}"
          role="button"
          tabindex="0"
          aria-label="Continue listening to ${escapeHtml(podcast.title)}"
        >
          <div class="cover" style="background:${getCoverGradient(podcast)};">
            ${renderCardCover(podcast)}
          ...
          <div class="card-body">
            <div class="title-row">
              ...
              <div class="duration">${formatDuration(time)} / ${escapeHtml(podcast.duration)}</div>
            </div>
    ...
    els.continueListeningSection.style.display = 'block';
    bindLibraryCards();
  }

  function renderRecentlyPlayed() {
    const recentlyPlayedItems = state.recentlyPlayed
      .map(id => state.podcasts.find(p => p.id === id))
      .filter(Boolean)
      .slice(0, 5); // Show up to 5 recently played

    if (!recentlyPlayedItems.length) {
      els.recentlyPlayedSection.style.display = 'none';
      return;
    }

    // Update header with clear button
    const header = els.recentlyPlayedSection.querySelector('.section-header');
    header.innerHTML = `
      <div>
        <h2 class="section-title">Recently Played</h2>
        <p class="section-hint">Your listening history.</p>
      </div>
      <button class="icon-btn clear-btn" title="Clear recently played">🗑️</button>
    `;
    ...
    els.recentlyPlayedContent.innerHTML = recentlyPlayedItems.map(podcast => `
      <article
        class="card-surface"
        data-open="${escapeHtml(podcast.id)}"
        role="button"
        tabindex="0"
        aria-label="Play ${escapeHtml(podcast.title)}"
      >
        <div class="cover" style="background:${getCoverGradient(podcast)};">
          ${renderCardCover(podcast)}
        </div>
    ...
    els.recentlyPlayedSection.style.display = 'block';
    bindLibraryCards();
  }
  ...
  function toggleFavorite(podcastId) {
    ...
    renderFavorites();
    renderLibrary(); // Re-render to update favorite buttons
    renderCategoryPageIfActive();
  }

  function renderFavorites() {
    const favoritePodcasts = state.favorites
      .map(id => state.podcasts.find(p => p.id === id))
      .filter(Boolean);

    if (!favoritePodcasts.length) {
      els.favoritesSection.style.display = 'none';
      return;
    }

    els.favoritesContent.innerHTML = favoritePodcasts.map(renderPodcastCard).join('');
    els.favoritesSection.style.display = 'block';
    bindLibraryCards();
  }

  function renderCategoryPageIfActive() {
    const activePage = document.querySelector('.page.active');
    if (activePage && activePage.id !== 'homePage') {
      const categoryId = activePage.id.replace('Page', '');
      if (['technology', 'science', 'geography', 'history'].includes(categoryId)) {
  ...
  function openDetail(podcast) {
    ...
    els.detailModal.dataset.podcastId = podcast.id;
  ...
  function closeDetail() {
    ...
    delete els.detailModal.dataset.podcastId;
  }
  ...
  function setMiniPlayer(podcast) {
    ...
    els.miniSkipBackBtn.disabled = false;
    els.miniSkipForwardBtn.disabled = false;
    els.miniSpeedBtn.disabled = false;
  ...
  function resetMiniPlayer() {
    ...
    els.miniSkipBackBtn.disabled = true;
    els.miniSkipForwardBtn.disabled = true;
    els.miniSpeedBtn.disabled = true;
    els.miniPlayPauseBtn.textContent = '▶';
    state.playbackRate = 1;
    if (els.miniSpeedBtn) els.miniSpeedBtn.textContent = '1x';
  }
  ...
  function playPodcast(podcast) {
    ...
    els.audioPlayer.playbackRate = state.playbackRate || 1;

    if (els.audioPlayer.src !== src) {
      els.audioPlayer.src = src;
    }

    const resumeTime = getPlaybackTime(podcast.id);
    if (resumeTime > 0) {
      els.audioPlayer.currentTime = resumeTime;
    }
  ...
  function wireEvents() {
    ...
    els.technologyBackBtn.addEventListener('click', () => openPage('home'));
    els.scienceBackBtn.addEventListener('click', () => openPage('home'));
    els.geographyBackBtn.addEventListener('click', () => openPage('home'));
    els.historyBackBtn.addEventListener('click', () => openPage('home'));
    ...
    els.detailPlayBtn.addEventListener('click', () => {
      const podcastId = els.detailModal.dataset.podcastId;
      const podcast = state.current || state.podcasts.find((item) => item.id === podcastId);
      if (podcast) {
        playPodcast(podcast);
        closeDetail();
      }
    ...
    els.miniSkipBackBtn.addEventListener('click', () => {
      if (state.currentKind === 'episode' && state.current) {
        const currentIndex = state.podcasts.findIndex((item) => item.id === state.current.id);
        if (currentIndex > 0) playPodcast(state.podcasts[currentIndex - 1]);
      }
    });

    els.miniSkipForwardBtn.addEventListener('click', () => {
      if (state.currentKind === 'episode' && state.current) {
        const currentIndex = state.podcasts.findIndex((item) => item.id === state.current.id);
        if (currentIndex < state.podcasts.length - 1) playPodcast(state.podcasts[currentIndex + 1]);
      }
    });

    els.miniSpeedBtn.addEventListener('click', () => {
      state.playbackRate = state.playbackRate === 1 ? 2 : state.playbackRate === 2 ? 3 : 1;
      if (state.currentKind === 'episode') {
        els.audioPlayer.playbackRate = state.playbackRate;
      }
      els.miniSpeedBtn.textContent = `${state.playbackRate}x`;
    });

    els.audioPlayer.addEventListener('timeupdate', () => {
      if (state.current && state.currentKind === 'episode') {
        savePlaybackTime(state.current.id, els.audioPlayer.currentTime);
      }
    });
    ...
    els.audioPlayer.addEventListener('ended', () => {
      if (state.current && state.currentKind === 'episode') {
        // Remove from continue listening when fully played
        delete state.playbackTimes[state.current.id];
        localStorage.setItem('playbackTimes', JSON.stringify(state.playbackTimes));
      ...
      if (state.current) {
        switch (event.key) {
          case ' ':
            ...
            break;
          case 'ArrowLeft':
            if (event.ctrlKey && state.currentKind === 'episode') {
              event.preventDefault();
              els.audioPlayer.currentTime = Math.max(0, els.audioPlayer.currentTime - 15);
            }
            break;
          case 'ArrowRight':
            if (event.ctrlKey && state.currentKind === 'episode') {
              event.preventDefault();
              els.audioPlayer.currentTime = Math.min(els.audioPlayer.duration, els.audioPlayer.currentTime + 15);
            }
            break;
        }
      }
